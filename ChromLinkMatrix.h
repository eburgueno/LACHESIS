 ...
 * A cluster generally represents a group of contigs which the GenomeLinkMatrix's clustering algorithm has clustered together.  The cluster may also represent
 * a true chromosome, split up into _N_contigs contigs, each of size _contig_size; this called a "non-de novo CLM" and is used for algorithmic testing.
 *
 * A ChromLinkMatrix contains a 2-D array of data, of size (2*_N_contigs) x (2*_N_contigs).  There are two bins for each contig, corresponding to the
 * forward and reverse orientation of each contig.  Each bin contains a vector<int> representing the set of distances of all links between the two contigs,
 * assuming a particular orientation.
 *
 * ChromLinkMatrices take their data from SAM files.  You can load data from SAM files using LoadFromSAM...().
 *
 * The goal of the ChromLinkMatrix class is to find a oriented ordering of contigs (a ContigOrdering object) that is best supported by the Hi-C links.
 * The Make...Order() functions employ a graph-based optimization algorithm to find the best ordering of contigs.  The OrientContigs() function implements the
 * WDAG-based contig orientation algorithm.
 *
 *
 *
 *
...
class ChromLinkMatrix
{
 public:
  ...
  ChromLinkMatrix( const string & species, const int contig_size, const int chrom_len );
  ...
  ChromLinkMatrix( const string & species, const int cluster_size );
  ...
  ChromLinkMatrix( const string & species, const vector<string> & SAM_files, const string & RE_sites_file, const ClusterVec & clusters, const int cluster_ID );
  ...
  void WriteFile( const string & CLM_file, const bool heatmap = false ) const;
  ...
  int NLinks( const int contig1, const int contig2 ) const { return _matrix[ 2*contig1 ][ 2*contig2 ].size(); }
  ...
  // If oriented = true, takes orientation into account.  oriented=false is faster.  Scores with and without orientation can't be directly compared.
  ...
  double OrderingScore( const ContigOrdering & order, const bool oriented = true, const int shred_start = -1, const int shred_stop = -1 ) const;
  // EnrichmentScore: Find the "enrichment", the degree to which the Hi-C links between contigs are between close contigs.  It's analogous to the concentration
  // of signal along the main diagonal of the heatmap.
  double EnrichmentScore( const ContigOrdering & order ) const;
  
  /* PRE-PROCESSING FUNCTIONS */
  
  // FindLongestContig(): Fill _longest_contig.  This function should be called right after loading in _contig_lengths.
  void FindLongestContig() { _longest_contig = *( max_element( _contig_lengths.begin(), _contig_lengths.end() ) ); }
  void SetCPScoreDist( const int CP_score_dist ) { _CP_score_dist = CP_score_dist; }
 ...
 private:
  ...
  // AddToMatrix: Add a individual Hi-C link to the matrix.  This function is only used when loading data from SAM files.
  void AddLinkToMatrix( const int contig1, const int contig2, const int read1_dist1, const int read1_dist2, const int read2_dist1, const int read2_dist2 );
  ...
  // "Repetitiveness factors" for each contig: the number of total links involving this contig, divided by the average.  Used in normalization.
  ...
  friend void LoadDeNovoCLMsFromSAM( const        string  & SAM_file,  const string & RE_sites_file, const ClusterVec & clusters, vector<ChromLinkMatrix *> CLMs );
  friend void LoadDeNovoCLMsFromSAM( const vector<string> & SAM_files, const string & RE_sites_file, const ClusterVec & clusters, vector<ChromLinkMatrix *> CLMs );
};
...
void LoadDeNovoCLMsFromSAM( const        string  & SAM_file,  const string & RE_sites_file, const ClusterVec & clusters, vector<ChromLinkMatrix *> CLMs );
void LoadDeNovoCLMsFromSAM( const vector<string> & SAM_files, const string & RE_sites_file, const ClusterVec & clusters, vector<ChromLinkMatrix *> CLMs );



// LoadNonDeNovoCLMsFromSAM: Import one or more SAM/BAM files and create a set of non-de novo ChromLinkMatrices corresponding to each chromosome.
// As many or as few of the ChromLinkMatrix pointers can be non-NULL.  Whichever ones are non-NULL will be assumed to represent ChromLinkMatrices with chrID
// equal to their index in the vector, and will be filled accordingly.
// This is "cheating" because we're using the true location of each contig to group them into clusters.  Its primary use is for heatmaps (DrawHeatmap).
// If you are creating a set of ChromLinkMatrices for each chromosome, this is much faster than calling LoadFromSAMNonDeNovo individually for each
// ChromLinkMatrix object because it only reads through the SAM file(s) once.
void LoadNonDeNovoCLMsFromSAM( const        string  & SAM_file,  vector<ChromLinkMatrix *> CLMs );
void LoadNonDeNovoCLMsFromSAM( const vector<string> & SAM_files, vector<ChromLinkMatrix *> CLMs );
